...
public class MinAreaRectangle {
    public double minAreaFreeRect(int[][] points) {
        ...
        for(int i=0; i < points.length; i++){
            for(int j=i+1; j < points.length; j++){
                ...
                double[] cen = centre(points[i], points[j]);
                double dis = distanceForSet(points[i], points[j]);
                String key = "" + dis+"."+cen[0] + "."+cen[1];
        ...
        for(String key : map.keySet()){
            ...
            for(int i=0; i < list.size(); i++){
                for(int j=i+1; j < list.size(); j++){
                    ...
                    int[] point2 = points[list.get(j)[0]];
                    int[] point3 = points[list.get(j)[1]];
    ...
    double distanceForSet(int[] point1, int[] point2){
        double  x = point1[0] - point2[0];
        double y = point1[1] - point2[1];
        return (x*x + y*y);
    }

    double[] centre(int[] point1, int[] point2){


        double x = (double)(point1[0] + point2[0] )/2;
        double y = (double)(point1[1] + point2[1] )/2;
    ...
    @Test
    public void test(){
        int[][] p = new int[][]{{3,1},{1,1},{0,1},{2,1},{3,3},{3,2},{0,2},{2,3}};
        System.out.println(minAreaFreeRect(p));
        System.out.println(minAreaFreeRect2(p));
